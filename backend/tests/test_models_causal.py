...
def test_iptw_happy_path_ate(client, sid):
    ...
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["method"] == "iptw"
    assert body["estimand"] == "ate"
    assert body["n_treated"] > 0 and body["n_control"] > 0
    ws = body["weight_summary"]
    assert ws["min"] > 0 and ws["max"] >= ws["min"]
    assert ws["ess_treated"] > 0 and ws["ess_control"] > 0
    assert set(body["smd_after"].keys()) == set(COVS)
    res = body["outcome_result"]
    assert res is not None and "error" not in res
    assert res["type"].startswith("weighted_glm")
    assert res["coefficients"][1]["variable"] == "treat"


def test_iptw_estimand_att(client, sid):
    r = client.post("/api/models/iptw", json={
        "session_id": sid, "treatment_col": "treat",
        "covariates": COVS, "outcome_col": "outcome", "estimand": "att",
    })
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["estimand"] == "att"
    # ATT: treated keep weight 1 → max_treated should be ~1.
    assert body["weight_summary"]["max_treated"] == pytest.approx(1.0, abs=1e-6)
...
def test_iptw_unstabilized_hard_truncation(client, sid):
    r = client.post("/api/models/iptw", json={
        "session_id": sid, "treatment_col": "treat",
        "covariates": COVS, "outcome_col": "outcome",
        "stabilize": False, "weight_truncation": "hard",
        "weight_truncation_max": 5.0,
    })
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["stabilize"] is False
    assert body["weight_truncation"]["mode"] == "hard"
    assert body["weight_summary"]["max"] <= 5.0 + 1e-6


def test_iptw_no_truncation(client, sid):
    r = client.post("/api/models/iptw", json={
        "session_id": sid, "treatment_col": "treat",
        "covariates": COVS, "outcome_col": "outcome",
    ...
    assert r.status_code == 200, r.text
    assert r.json()["weight_truncation"]["mode"] == "none"


def test_iptw_bootstrap_se(client, sid):
    r = client.post("/api/models/iptw", json={
        "session_id": sid, "treatment_col": "treat",
        "covariates": COVS, "outcome_col": "outcome",
        "se_method": "bootstrap", "bootstrap_reps": 60,
    })
    assert r.status_code == 200, r.text
    body = r.json()
    assert body["se_method"] == "bootstrap"
    res = body["outcome_result"]
    assert res is not None and "error" not in res
    # Bootstrap suffix appended when it succeeds; otherwise robust type kept.
    assert "weighted_glm" in res["type"]
...
def test_iptw_non_binary_treatment_422(client, sid):
    r = client.post("/api/models/iptw", json={
        "session_id": sid, "treatment_col": "AGE", "covariates": ["LDL"],
    })
    assert r.status_code == 422, r.text


def test_iptw_missing_column_422(client, sid):
    r = client.post("/api/models/iptw", json={
        "session_id": sid, "treatment_col": "treat",
        "covariates": ["AGE", "MISSING"],
    })
    assert r.status_code == 422, r.text
