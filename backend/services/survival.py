"""Survival-analysis service: pure model-fitting for the survival endpoints.

Extracted from routers/models_survival.py so the router is a thin HTTP adapter.
...
Public API:
    fit_kaplan_meier(df_full, req)
    fit_cox(df_full, req)
    fit_cox_tv(df_full, req)
    fit_rcs(df_full, req)
    fit_cox_rcs(df_full, req)
"""
...
# compute_vif is a shared modelling helper (pure pandas/statsmodels); reused here.
from routers._models_shared import compute_vif as _compute_vif
...
def fit_kaplan_meier(df_full, req):
    n_total = len(df_full)
