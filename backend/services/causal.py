"""Causal-inference service: pure compute for PSM and IPTW.

Extracted from routers/models_causal.py so the router is a thin HTTP adapter.
These functions take an already-loaded DataFrame and a duck-typed request
object (any object exposing the documented attributes) and return plain dicts;
they perform NO session/store I/O and import nothing from the routers package.

Public API:
    run_psm(df_full, req)  -> (result: dict, matched_export_df: pd.DataFrame)
    run_iptw(df_full, req) -> (result: dict, audit: dict)
plus the reusable statistical helpers (SMD, propensity scores, matching,
Rosenbaum bounds, IPTW weights, weighted balance).
"""
...
def run_psm(df_full, req):
    ...
    return result, df_export


# ── IPTW statistical helpers ──────────────────────────────────────────────────
