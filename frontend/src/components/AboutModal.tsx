...
const CHANGELOG = [
  { ver: "3.1.0", date: "2026-06-01", notes: "Maintainability, test-coverage, CI, and privacy hardening release (no change to statistical results). (1) Frontend god-file decomposition — ModelsPanel.tsx cut from 2,509 → 675 LOC: coefficient/OR tables, the forest plot, prediction + coefficient-detail views split into focused components/models/* files, derived column metadata moved into a useModelData hook, and ~410 lines of dead KM/Cox render code removed (those analyses live in Survival Advanced). DataTable.tsx reduced (Select-Cases and Value-Labels modals extracted into components/datatable/*). (2) Backend services layer extended — survival_advanced.py thinned 2,447 → 271 LOC: all 13 endpoints now delegate to services/survival_advanced_service.py. (3) Test coverage 48% → 64% — 238 endpoint/contract tests added across compute, logistic/survival/causal models, sessions, stat-utils, stats, and advanced survival; surfaced and fixed a real /api/models/lmm crash (NaN AIC/BIC under REML broke JSON serialisation). (4) CI hardening — new GitHub Actions workflow runs backend pytest + coverage and frontend tsc/build on every push/PR, with a coverage-floor ratchet (60%) and a guard that fails the build if any tracked file exceeds 5 MB. (5) Dependency hygiene — requirements split into runtime vs requirements-dev, loose pins tightened. (6) Resilience — panel-level React error boundary now shows a contained, recoverable fallback instead of white-screening; fixed a Decision Curve tab crash (columns read from the wrong store field). (7) Repo hygiene — removed a 1 GB build/ artifact directory and hardened .gitignore. (8) Privacy — removed the MapMyVisitors third-party visitor-tracking widget; uSTAT now loads no third-party analytics/tracking scripts." },
  { ver: "3.0.0", date: "2026-06-01", notes: "Major architectural milestone: complete god-file decomposition of the original 4700+ line models.py into a clean, maintainable services layer (20+ focused pure modules) with thin routers. Full 13-phase advanced clinical biostatistics roadmap delivered: (1) PSM with Rosenbaum bounds, (2) RCS with interactions, (3-6) Missing data (MICE + delta-adjustment MNAR sensitivity), model validation (bootstrap optimism), causal sensitivity (E-value + QBA), (7-8) Advanced survival (shared frailty, multistate illness-death + dynamic prediction, two-stage joint models), (9) External validation framework (C-index, calibration, IPCW IBS, time-dependent AUC, transportability diagnostics), (10-12) Survival ML benchmark (GB ranking + permutation importance) with deep Phase 9 integration (risk scores → survival probabilities → full IBS/tdAUC on shifted cohorts), (13) Professional Decision Curve Analysis service (binary + survival modes, standardized net benefit, interventions avoided, rich assumptions/warnings/result_text) with flexible router support for pre-computed risk scores and full frontend integration in SurvivalAdvancedPanel (net benefit plots, summary cards). All new work follows strict conventions: immutability, simulation-based TDD, rich API responses. Version bump to 3.0.0 marks the transition to a production-grade modular platform." },
  { ver: "2.6.0", date: "2026-05-30", notes: "Major analytical upgrade introducing exploratory factor analysis, Bayesian hypothesis testing, advanced reliability, interactive data cleaning, and automated R script replication: (1) Factor Analysis & PCA — Exploratory Factor Analysis (EFA) & Principal Component Analysis (PCA) featuring Bartlett's test of sphericity, Kaiser-Meyer-Olkin (KMO) MSA suitability check, orthogonal (Varimax) and oblique (Promax) rotations, loadings tables, and interactive Plotly Scree + 2D Loadings plots. (2) Bayesian Statistics — JASP-like JZS Bayes Factors (BF10 and BF01) for One-sample, Independent, and Paired t-tests, Pearson Correlation (Fisher-z prior), and Multiple Linear Regression (BIC-based approximation). Includes detailed interpretation guidelines and overlays comparing prior Cauchy curves with posterior densities (Savage-Dickey ratio). (3) McDonald's Omega (ω) — Calculated using factor analysis on standardized items and displayed in the Reliability panel side-by-side with Cronbach's alpha. (4) Data Cleaning Tools — Live Listwise Deletion, IQR/Z-score Outlier Removal, and global Find & Replace implemented within the Compute panel. (5) R Replication Hub — Replaced the Sandbox with a dedicated R replication suite compiling a single, clean, cohesive R script replicating every step in the user's active session history. (6) Usability Polish — Orange subset-filtering badge added to the header next to the filename when active filter conditions are applied." },
  { ver: "2.5.0", date: "2026-05-30", notes: "Milestone release consolidating the advanced-methods wave (v2.3.0–2.4.2) — uSTAT now spans 90+ tests & methods. Highlights since 2.2.x, all built on the existing SciPy / statsmodels / scikit-learn / lifelines stack (no new dependencies): (1) Machine learning — Random Forest & Gradient Boosting (classification/regression) with out-of-fold cross-validated AUC + bootstrap CI, calibration, confusion matrix, and impurity + permutation importance (Models → Machine Learning). (2) Time series — ARIMA/SARIMA with auto-AIC order + forecast, STL/classical decomposition, ADF+KPSS stationarity with ACF/PACF (Models → Time Series). (3) Meta-analysis — fixed/random effects (DL & Paule-Mandel τ²), prediction interval, subgroup, meta-regression, and publication-bias diagnostics (Egger/Begg/funnel/trim-and-fill), with raw-2×2 or effect+CI/SE input (new Meta tab). (4) Weighted (survey) descriptives — weighted mean/SD/CI/quartiles, Kish ESS, weighted proportion + weighted t-test (Summary → Weighted). (5) Recurrent events — LWYY (modified Andersen-Gill with cluster-robust sandwich SE) + mean cumulative function (Survival Advanced → Recurrent Events). (6) Multistage gatekeeping — truncated Hochberg/Holm, serial/parallel, for hierarchical endpoint testing (Tests → Gatekeeping). (7) Non-inferiority margin testing — one-sided α ↔ two-sided (1−2α) CI, RR/RD/OR/mean-difference vs a prespecified margin (Tests → Non-Inferiority). Every addition is cross-validated where applicable, audit-logged, surfaced in the header search, and recognised by the Methods-appendix DOCX export." },
  { ver: "2.4.2", date: "2026-05-30", notes: "Non-inferiority margin testing added (the trial-design analysis behind the 'ITT' question — supply the ITT or per-protocol dataset and test against a prespecified margin). New POST /api/stats/noninferiority: a one-sided α corresponds to a two-sided (1−2α) CI — α=0.05 ↔ 90% CI, the regulatory non-inferiority convention (matches both FDA and EMA guidance). Binary outcome supports a risk ratio, risk difference, or odds ratio (log-Wald CIs for RR/OR with zero-cell correction, Wald for RD); continuous outcome gives a Welch mean difference. Non-inferiority is concluded from the relevant CI bound vs the margin: bound 'upper' → non-inferior if the upper (1−2α) CI bound < margin (harmful event, e.g. the RR CRT-DX/CRT-D < 1.20 example); bound 'lower' → non-inferior if the lower bound > margin (preserve benefit). Returns the effect estimate, the (1−2α) CI, the one-sided non-inferiority p-value, per-arm counts/means, and a plain-English verdict. Frontend: new Non-Inferiority sub-tab under Tests — binary/continuous toggle, outcome + arm pickers (test vs reference auto-populated from the group's two levels), effect measure, margin, bound, one-sided α (with a live '→ two-sided XX% CI' readout), and a green/amber demonstrated-or-not verdict banner. NOTE: the Andersen-Gill recurrent-event model with a robust sandwich variance estimator (LWYY) shipped in 2.4.0 (Survival Advanced → Recurrent Events); ANCOVA is under Tests → Hypothesis. Search box indexes non-inferiority / margin / equivalence / ITT / 90% CI (+ Turkish üstünlük dışılık). Methods appendix recognises the action." },
  { ver: "2.4.1", date: "2026-05-30", notes: "Multistage gatekeeping for hierarchical hypothesis testing added (second of the two requested trial-methods, after LWYY) — controls family-wise error across ordered families of endpoints (e.g. primary → secondary) using a truncated Hochberg (step-up) or truncated Holm (step-down) procedure within each family (Dmitrienko, Tamhane & Wiens 2008). New POST /api/multiplicity/gatekeeping (pure python). Within-family critical fractions c_i = γ/(m−i+1) + (1−γ)/m: γ = 1 is the ordinary Hochberg/Holm, γ = 0 is Bonferroni, and a truncation < 1 in a non-terminal family reserves Bonferroni mass so the gate to the next family can open while still testing within the family (default γ = 0.5 non-terminal, 1.0 terminal). Between-family logic: serial (next family tested only if EVERY prior hypothesis is rejected) or parallel (if AT LEAST ONE in the preceding family is rejected). Multiplicity-adjusted p-values are returned as the smallest α at which each hypothesis is rejected by the full procedure (exact to a 1/2000 grid; reject when adjusted p ≤ α). Frontend: new Gatekeeping sub-tab under Tests — editable ordered families (add/remove families and hypotheses, per-family γ override), method / logic / α pickers, a sample primary+secondary endpoint setup, and a results table per family (raw p, adjusted p, Reject/Retain) with CSV/XLSX export. Search box indexes gatekeeping / Hochberg / Holm / multiplicity / hierarchical / FWER (+ Turkish çoklu test). Methods appendix recognises the action." },
  { ver: "2.4.0", date: "2026-05-29", notes: "Recurrent-event analysis added — the LWYY (Lin-Wei-Yang-Ying) model, i.e. a modified Andersen-Gill Cox model with a cluster-robust variance estimator, for endpoints that recur within a subject (e.g. repeat heart-failure hospitalisations). New POST /api/survival_advanced/recurrent_lwyy: fits the counting-process (start, stop] intervals via lifelines CoxPHFitter with entry_col = start, duration_col = stop, cluster_col = subject id, robust = True — the point estimate is Andersen-Gill and the SE is the LWYY cluster-robust sandwich that accounts for within-subject event correlation. exp(β) is reported as a rate ratio (ratio of event rates / mean cumulative functions), not a single-event hazard ratio. Output: rate-ratio table (RR, 95% CI, robust SE, z, p), subject / event / events-per-subject counts, total follow-up, C-index, and a nonparametric mean cumulative function (MCF, Nelson 1995) plotted overall or by an optional group column. Frontend: a new 'Recurrent Events (LWYY)' section under Survival Advanced with the standard 3-column resizable layout (id / start / stop / event / predictors / group on the left, MCF curve centre, RR table + interpretation right). Search box indexes lwyy / Andersen-Gill / recurrent / mean cumulative function / rate ratio (+ Turkish: tekrarlayan). Methods appendix recognises the action. (Next: truncated-Hochberg gatekeeping for hierarchical endpoint testing.)" },
  { ver: "2.3.3", date: "2026-05-29", notes: "Weighted (survey) descriptive statistics added (Sprint 1.4 — completes Sprint 1 of the roadmap). New POST /api/stats/weighted_descriptive (statsmodels DescrStatsW): per-variable weighted mean, SD, SE, 95% CI, weighted quartiles (interpolated on the cumulative-weight grid), and Kish's effective sample size ESS = (Σw)²/Σw². A binary 0/1 variable additionally returns a Horvitz-Thompson weighted proportion with CI. Supplying a 2-level group column adds a weighted mean difference + Welch-style two-sample weighted t-test (statsmodels CompareMeans). Weights-only design — full complex-survey strata/cluster is not yet modelled (documented in the result assumptions). Frontend: new 'Weighted (survey)' sub-tab under Summary — weight-column picker, value-variable multiselect, optional group, results table (weighted mean / SD / CI / median / IQR / proportion + ESS) with CSV/XLSX export and a weighted-comparison card. Search box indexes weighted mean / survey / sampling weights / Kish / Horvitz (+ Turkish: ağırlıklı). Methods appendix recognises the action. This closes the 'weighted analysis weak' gap from the critique — combined with the IPTW weighted GLM + weighted Cox already in PSM." },
  { ver: "2.3.2", date: "2026-05-29", notes: "Full meta-analysis suite added (Sprint 1.3 of the roadmap) — the DerSimonian-Laird forest that previously lived inside the chart builder is now a first-class study-level tool under a new Meta top-tab. New `/api/meta` router (pure numpy/scipy/statsmodels): (1) /analyze — fixed + random-effects pooling with DerSimonian-Laird OR Paule-Mandel τ², Cochran Q, I², H², per-study inverse-variance weights, and a 95% prediction interval (Higgins 2009); studies entered as effect+95%CI, effect+SE, OR a raw 2×2 table (events/n per arm → log-OR / log-RR / risk difference with a 0.5 continuity correction for zero cells). (2) /subgroup — pools each subgroup separately + a between-subgroup Q test for effect-modification. (3) /regression — meta-regression of the effect on a study-level moderator via weighted least squares (1/(v+τ²) weights) with a bubble plot. (4) /bias — Egger's regression test, Begg's rank correlation, a funnel plot, and a Tweedie L0 trim-and-fill estimate of missing studies. Frontend: a new Meta panel with an editable study table (effect+CI / effect+SE / 2×2 column modes), TSV/CSV paste, a sample dataset, and four one-click analyses (Forest + pool / Subgroup / Meta-regression / Publication bias) each with its plot (forest with pooled diamond, subgroup table, regression bubble, funnel) and a results column. Methods appendix recognises the new actions; search box indexes meta-analysis / subgroup / meta-regression / Egger / Begg / funnel / trim-and-fill (+ Turkish: meta analiz, alt grup, yayın yanlılığı). Measures: OR / RR / HR / RD / MD / SMD." },
  { ver: "2.3.1", date: "2026-05-29", notes: "Time-series analysis added (Sprint 1.2 of the advanced-features roadmap) — answers the 'no ARIMA / time series' gap. New `/api/timeseries` router, pure statsmodels (no new packages). Three tools under a new Time Series sub-tab in Models: (1) ARIMA / SARIMA — `SARIMAX` fit with manual (p,d,q)(P,D,Q,s) order or an `auto` bounded AIC grid search over p,d,q ∈ {0,1,2}×{0,1}×{0,1,2}; returns the coefficient table, AIC/BIC, a Ljung-Box white-noise test on the residuals, the in-sample fitted line, and an n-step forecast with a 95% confidence band. (2) Seasonal decomposition — STL (robust) or classical additive/multiplicative, plotted as stacked observed / trend / seasonal / residual panels with Hyndman-Athanasopoulos trend and seasonal strength scores. (3) Stationarity — ADF (H₀: unit root) + KPSS (H₀: stationary) with a combined verdict, plus ACF and PACF stem plots with 95% bands to read candidate AR/MA orders. An optional time column orders the series (datetime auto-parsed, else numeric, else row order). 3-column resizable layout (controls / plot / results), audit log + Methods appendix recognise the new actions, and the search box indexes arima / sarima / stl / adf / kpss / acf / pacf (+ Turkish: zaman serisi, durağanlık, mevsimsellik, tahmin)." },
  ...
  { ver: "2.5.0", date: "2026-06-07", notes: "Multiple-imputation pooling extended to the survival analyses. Fine-Gray competing-risks regression, landmark Cox regression, and RMST/ΔRMST now support an imputation='mice' option that fits the model on m chained-PMM datasets and pools by Rubin's rules — Fine-Gray on the log-subdistribution-hazard scale, landmark on the log-HR scale, RMST on the raw value/difference scale (imputing only the group covariate; survival time/event are kept complete-case, per best practice). A new correct within+between pooler (services.missing_data.pool_rubin_terms — total variance T = Ubar + (1+1/m)·B, Barnard–Rubin degrees of freedom) reports the fraction of missing information per estimate. Survival Advanced panel gains a 'Complete-case / MICE (pooled)' toggle on these three analyses, shows the pooling note, and adds an FMI column to the landmark Cox table." },
  { ver: "2.4.0", date: "2026-06-07", notes: "Missing-data & multiple-imputation overhaul. (1) Imputation engine rebuilt as real chained-equation Predictive Mean Matching (PMM) — continuous variables draw a random donor from the nearest predicted means (non-parametric, only ever imputes values that actually occurred), binary variables use a logistic-style metric, categorical variables use hot-deck. This replaces the parametric sklearn IterativeImputer behind the model panels' MICE option, so the m completed datasets feeding Rubin's-rules pooling (Linear / Logistic / Cox) are now mice-faithful. (2) MICE no longer crashes or hard-fails on categorical columns: the Missing-Data panel imputes numeric → MICE and categorical → most-frequent (mode); the column right-click 'MICE' routes text to mode instead of an IndexError, and all-NaN feature columns are guarded. (3) Missing-Data panel rewritten: variables shown as a list (not a tile gallery) with inline per-column Mean/Median/Mode/MICE quick-impute; 'Let AI assess' (which needed an AI key) replaced by 'Analyze missingness' that runs Little's MCAR test + a Welch-t dependence heuristic and prints plain-language MCAR-vs-MAR recommendations; a sensitivity panel wires the previously-orphaned complete-case-vs-MICE comparison (KS distribution check); MICE gains a configurable seed. (4) Data-tab 'Fill blanks' errors are now surfaced instead of silently swallowed. New endpoints: /api/compute/{id}/missing_diagnostics, fill_blanks __mode__." },
  { ver: "2.3.0", date: "2026-06-07", notes: "Ordinal (ordered categorical) measurement level — SPSS's third 'Measure' alongside Scale and Nominal. A column can now be marked 'Ordered Categorical' (teal 'ord' badge) by cycling its type in the Data tab; the kind persists across reload/undo and exports to SPSS with measure='ordinal'. Ordinal columns are numeric-coded ordered data, so they stay eligible in BOTH numeric and categorical variable pickers across every panel (eligibility now flows through isNumericKind/isCategoricalKind helpers). Order-aware tests detect an ordinal selection and suggest the right method with one-click switch: Correlation → Spearman ρ (over Pearson); Hypothesis group tests → Jonckheere-Terpstra trend (over Kruskal-Wallis/ANOVA); Categorical tests → Cochran-Armitage trend (for an ordered exposure vs binary outcome); Regression → Ordinal Logistic. The /api/models/ordinal endpoint was rebuilt as a TRUE proportional-odds model (statsmodels OrderedModel) — one OR (95% CI), p per predictor shared across the cumulative thresholds, plus the cut-points and McFadden pseudo-R² — replacing the previous multinomial approximation. New 'Ordinal Logistic (proportional odds)' model in the Regression panel reuses the OR coefficient table + forest plot; selecting an ordinal outcome surfaces a prompt to switch to it." },
  { ver: "2.2.0", date: "2026-06-07", notes: "Data-prep & reporting batch. (1) Robust date detection + conversion — dates pasted/imported from Excel or SPSS that landed as text (especially month-name forms like '5 Ocak 2024' or Excel serial numbers) are now recognised. New backend services/date_parser.py (ported from the drtr Excel-date tool) parses numeric separators, Turkish + English month names, Excel serial numbers (1899-12-30 epoch, 1900 leap-year bug), and 2-digit years against a century threshold, resolving DD/MM vs MM/DD ambiguity by scanning the whole column. New POST /api/compute/{sid}/parse_dates converts a column to real datetime64 (ISO) in place — so Survival / Time-Series read it directly — with a preview_only mode. UI: right-click a column → '📅 Parse as date…' opens a modal with auto/day-first/month-first + century-cutoff options and a live raw→parsed preview + parse counts. Import auto-detection (_detect_kind) now also labels month-name date columns as 'date' (pure-numeric columns are never auto-labelled, so integer IDs / serials are safe). (2) Per-column Find & Replace — right-click a column → '🔁 Find & Replace…' maps each distinct value to a new one in place (no new column); when every value ends up numeric the column auto-casts to numeric, so e.g. kadın→0 / erkek→1 yields a real 0/1 predictor instead of text dummies. Value labels follow the mapping. (3) HR Table (Uni + Multi) — a Cox survival counterpart to the OR Table in the Regression panel: each predictor's univariable HR, its parsimonious-model HR (a user-ticked ★ subset), and its fully-adjusted HR side by side (publication Table 3), with per-predictor reference selection for categorical predictors and a copy-table action. (4) Survival Advanced method navigation switched from large cards to a compact left radio list matching the Regression panel's model selector." },
...
const METHODS: MethodGroup[] = [
  ...
  },
  {
    group: "Time series",
    items: [
      { name: "ARIMA / SARIMA (manual or auto-AIC order) + forecast", impl: "statsmodels.tsa.statespace.SARIMAX" },
      { name: "Ljung-Box residual white-noise test", impl: "statsmodels.stats.diagnostic.acorr_ljungbox" },
      { name: "Seasonal decomposition (STL / classical)", impl: "statsmodels.tsa.seasonal.STL / seasonal_decompose" },
      { name: "Stationarity (ADF + KPSS) + ACF / PACF", impl: "statsmodels.tsa.stattools.adfuller / kpss / acf / pacf" },
    ],
  },
  {
    group: "Meta-analysis",
    items: [
      { name: "Fixed + random effects (DL / Paule-Mandel τ²) + prediction interval", impl: "custom (inverse-variance) + scipy" },
      { name: "Effect from 2×2 (OR / RR / RD) with continuity correction", impl: "custom" },
      { name: "Subgroup analysis + between-group Q", impl: "custom" },
      { name: "Meta-regression (moderator)", impl: "statsmodels.WLS" },
      { name: "Publication bias: Egger / Begg / funnel / trim-and-fill", impl: "statsmodels.OLS + scipy.kendalltau + Tweedie L0" },
    ],
  },
  {
    group: "Trial design",
    items: [
      { name: "Non-inferiority margin test (RR / RD / OR / mean diff)", impl: "log-Wald / Wald CI at 1−2α + one-sided p" },
      { name: "TOST equivalence (independent / paired / one-sample)", impl: "statsmodels.stats.weightstats.ttost" },
    ],
  },
  {
    group: "Multiplicity",
    items: [
      { name: "Multistage gatekeeping (serial / parallel)", impl: "custom (Dmitrienko-Tamhane-Wiens)" },
      { name: "Truncated Hochberg / Holm within family", impl: "custom (γ-truncated critical fractions)" },
      { name: "Multiplicity-adjusted p-values", impl: "infimum-α grid scan" },
    ],
  },
  {
    group: "Weighted / survey",
    items: [
      { name: "Weighted mean / SD / SE / 95% CI / quartiles + Kish ESS", impl: "statsmodels.stats.weightstats.DescrStatsW" },
      { name: "Weighted proportion (Horvitz-Thompson) for 0/1 variables", impl: "custom" },
      { name: "Weighted two-sample t-test (Welch)", impl: "statsmodels CompareMeans" },
    ],
  },
  {
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Recurrent events — LWYY (modified Andersen-Gill, robust SE)", impl: "lifelines.CoxPHFitter (entry + cluster_col + robust)" },
      ...
      { name: "Landmark analysis + dynamic prediction", impl: "lifelines + custom slicing" },
      ...
      { name: "Shared gamma frailty (cluster-level)", impl: "custom penalised Cox + moment estimator" },
      { name: "Multistate (illness-death) + state occupation probabilities + generalised Brier", impl: "custom Aalen-Johansen style + dynamic prediction from landmark" },
      { name: "Two-stage joint longitudinal-survival model", impl: "LMM (statsmodels) → random effects → Cox" },
      { name: "External validation (C-index, calibration slope, IPCW IBS, tdAUC, transportability)", impl: "services/external_validation.py (full Phase 9 framework)" },
      { name: "Survival ML benchmark (GB ranking + permutation importance) with Phase 9 integration", impl: "services/survival_ml.py (risk scores → survival probabilities → IBS/tdAUC)" },
  ...
  },
  {
    ...
    items: [
      ...
      { name: "Decision curve analysis (professional) — binary + survival, standardized net benefit, interventions avoided", impl: "services/decision_curve.py (Phase 13) — flexible pre-computed risk/probability input, rich assumptions/warnings" },
